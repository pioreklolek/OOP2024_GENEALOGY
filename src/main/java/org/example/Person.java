...
public class Person implements Serializable{
    ...
    public  static String generateTree(List<Person>people) {
        ...
        String peopleString = people.stream().map(p -> addObject.apply(p)).collect(Collectors.joining());

        String parentsString;
        people.stream().flatMap(person -> person.parents.isEmpty() ? Stream.empty() :
             parentsString = person.parents.stream()
                    .map(parent -> "\n" + addObject.apply(parent) + "\n" + cleanPersonName.apply(person))
                    .collect(Collectors.joining());
        );
    }
